<template>
  ...
  <span v-if="totalPercent !== 100" class="text-danger ms-2">
    (should be 100%)
  </span>
</div>
...
import { getGreetings, updateGreeting,  deleteGreeting } from "@/api/eventGreetings.api";
...
async function reload() {
  const res = await getGreetings(props.eventKey);
  greetings.value = res.data;
}

async function removeGreeting(g) {
  const ok = confirm("Delete this greeting?");
  if (!ok) return;
  ...
  try {
    ...
    console.error("Delete greeting failed", err);
